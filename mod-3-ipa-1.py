...
def vigenere_cipher(message, key):
    ...
    for x in range(word_length):
        ...
        if word[x]  == " ":
            ...
            index = alphabet.index(word[x])
            new_position = shift + index
            if new_position > 25:
                new_shift = new_position%26
                if  new_shift > 1 and new_shift + index < 25:
                    new_word = new_shift + index
                    word[x] = alphabet[new_word]

                if shift + index >= 25:
                    new_word = new_shift
                    word[x] = alphabet[new_word]
            else:
                new_word = new_position
                word[x] = alphabet[new_word]
    word = ''.join(word)
    return word
...
def scytale_cipher(message, shift):
    ...
    #check if length of message is a multiple of shift, if not then put underscore
    ...
    for i in range(0, message_length):
        ...
        index = (i // shift) + new_shift * (i % shift)
